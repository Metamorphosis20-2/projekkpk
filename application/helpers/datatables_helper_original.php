	...
	if(isset($buttonother)){
		...
		if($inline_button_other){
			...
			foreach($buttonother as $keyrow_buttonrow=>$valuerow_buttonrow){
				...
				// debug_array($valuerow_buttonrow);
				if(!is_numeric($key_buttonrow)){
					if(isset($valuerow_buttonrow[0])){
						...
						foreach($valuerow_buttonrow as $keyy=>$valuee){
							${$keyy} = $valuee;
						}
						$icon_row = $icon;
					}
					// debug_array($buttonother);
				}
			...
			foreach($buttonother as $keyrow_buttonrow=>$valuerow_buttonrow){
				...
				if(!is_nan($key_buttonrow)){
					$alt = $keyrow_buttonrow;
					if(isset($valuerow_buttonrow[1])){
						$icon_row = str_replace("fa-", "", $valuerow_buttonrow[1]);
					}
					if(isset($valuerow_buttonrow[2])){
					...
					if(isset($valuerow_buttonrow[4])){
						$buttonsize = $valuerow_buttonrow[4];
					}
				}
				...
				$arrButton = array_merge($arrButton, 
					array(
						$indexToolbar=>array(
							'text'=>$keyrow_buttonrow, 
							'image'=> "fas fa-" . $icon_row,
							'events'=>$function,
							'theme'=>$buttonclass,
							'width'=>$buttonsize
							)
			...
			$arrToolbar = array_merge($arrToolbar, array('arrButton'=>$buttonother));
		}
	}
	if($footer){
		$tfoot .= "</tr>";
